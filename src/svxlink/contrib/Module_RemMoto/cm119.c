...
//
//    Adapted for use to channel change on Motorola GM300 Series Radio (bitmasking) 
//    using the cm119 GPIO pins 
//    Copyright (C) 2025 Robert Middelmann VK5TRM 
//
...
int verbose = 0; // new flag
static int cm119_write (char *name, int iomask, int iodata);
// GPIO pin bit masks for CM108/cm119
#define GPIO_PIN5   (1 << 0)
#define GPIO_PIN6   (1 << 1)
#define GPIO_PIN7   (1 << 2)
#define GPIO_PIN8   (1 << 3)

int cm119_set_gpio_pins(char *name, int pin_mask, int state_mask)
{
    printf("[DEBUG] cm119_set_gpio_pins: name=%s, pin_mask=0x%X, state_mask=0x%X\n", name, pin_mask, state_mask);

    // Only allow pins 5-8
    const int allowed_mask = GPIO_PIN5 | GPIO_PIN6 | GPIO_PIN7 | GPIO_PIN8;

    // Remove any bits for pins outside 5-8
    pin_mask &= allowed_mask;
    state_mask &= allowed_mask;

    if (pin_mask == 0) {
        printf("%s No valid pins selected (only GPIO pins 5-8 allowed).\n", name);
        return -1;
    }
    if (pin_mask < 0 || pin_mask > 0xFF) {
        printf("%s Invalid pin mask %d. Must be between 0 and 255.\n", name, pin_mask);
        return -1;
    }
    if (state_mask < 0 || state_mask > 0xFF) {
        printf("%s Invalid state mask %d. Must be between 0 and 255.\n", name, state_mask);
        return -1;
    }

    if (verbose) printf("[DEBUG] Calling cm119_write with pin_mask=0x%X, state_mask=0x%X\n", pin_mask, state_mask);
    int ret = cm119_write(name, pin_mask, state_mask);
    if (verbose) printf("[DEBUG] cm119_write returned %d\n", ret);
    return ret;
}


int cm119_set_gpio_pin (char *name, int num, int state);
...
int cm119_inventory (struct thing_s *things, int max_things);
...
void print_inventory() {
	...
	num_things = cm119_inventory (things, MAXX_THINGS);
...
void print_help() {
    printf("cm119 -[hp] [-H <hid device>]\n"
           " -h: print this help\n"
           " -p: print attached USB devices\n"
           " -H <dev>: use <dev> for I/O\n"
           " -P <pin>: set on <pin>\n"
           " -L <1/0>: set <pin> to 0 (low) or 1 (high)\n"          
           " -M <mask>: decimal bitmask of pins to set HIGH (e.g., 21 sets pins 1,3,5)\n"
           " -v print DEBUG: Verbose output\n");
}



// ... (all your existing definitions, structs, and functions stay unchanged)
...
//    if (verbose) printf("\n");
    ...
    while ((opt = getopt(argc, argv, "hpH:P:L:M:vp")) != -1) {
        switch (opt) {
        case '?':
        case 'h':
            print_help();
    ...
    if (pin != -1 && level != -1) {
        if (!hiddev) {
            printf("-H must be provided with -P/-L.\n");
            return 1;
        }
        if (cm119_set_gpio_pin(hiddev, pin, level)) {
    ...
    if (mask != -1) {
        if (verbose) printf("[DEBUG] Entered mask-handling block\n");
        if (!hiddev) {
            printf("-H must be provided with -M.\n");
            return 1;
        }
        if (verbose) printf("[DEBUG] Raw mask input: %d (0x%X)\n", mask, mask);
        const int allowed_mask = GPIO_PIN5 | GPIO_PIN6 | GPIO_PIN7 | GPIO_PIN8;
        mask &= allowed_mask;
        if (verbose) printf("[DEBUG] Filtered mask (only GPIO5-8): %d (0x%X)\n", mask, mask);
        // Translate to actual iomask/iodata bits like cm119_set_gpio_pin
    int iomask = 0;
    int iodata = 0;
    
    if (mask == 0) {
        // Special case: clear all GPIO pins 5–8
        iomask = (1 << (5 - 1)) | (1 << (6 - 1)) | (1 << (7 - 1)) | (1 << (8 - 1));
        iodata = 0;  // all low
        if (verbose) printf("[DEBUG] Clearing all GPIO5 to GPIO8\n");
    } 
    else {
        // Set only the requested pins HIGH
    if (mask & GPIO_PIN5) { iomask |= (1 << (5 - 1)); iodata |= (1 << (5 - 1)); }
    if (mask & GPIO_PIN6) { iomask |= (1 << (6 - 1)); iodata |= (1 << (6 - 1)); }
    if (mask & GPIO_PIN7) { iomask |= (1 << (7 - 1)); iodata |= (1 << (7 - 1)); }
    if (mask & GPIO_PIN8) { iomask |= (1 << (8 - 1)); iodata |= (1 << (8 - 1)); }
    }  
      
    if (cm119_write(hiddev, iomask, iodata)) {
        printf("Failed setting GPIO pins.\n");
        return 1;
    }
        return 0;
  }
...
void cm119_find_ptt (char *output_audio_device, char *ptt_device,  int ptt_device_size)
{
	struct thing_s things[MAXX_THINGS];
	int num_things;
	int i;
...
/*-------------------------------------------------------------------
 *
 * Name:	cm119_set_gpio_pin
 *
 * Purpose:	Set one GPIO pin of the cm119 or similar.
 *
...
int cm119_set_gpio_pin (char *name, int num, int state)
{
	...
	if (num < 1 || num > 8) {
	  printf("%s cm119 GPIO number %d must be in range of 1 thru 8.\n", name, num);
	  return (-1);
	}

	if (state != 0 && state != 1) {
	  printf("%s cm119 GPIO state %d must be 0 or 1.\n", name, state);
	  return (-1);
	}
	...
	return (cm119_write (name, iomask, iodata));

}  /* end cm119_set_gpio_pin */
 ...
 * Name:	cm119_write
 *
 * Purpose:	Set the GPIO pins of the cm119 or similar.
 *
 * Inputs:	name		- Name of device such as /dev/hidraw2.
 *
 ...
 *		An application probably wants to use cm119_set_gpio_pin.
 *
 *------------------------------------------------------------------*/

static int cm119_write (char *name, int iomask, int iodata)
{
	if (verbose) printf("[DEBUG] Entering cm119_write: name=%s, iomask=0x%X, iodata=0x%X\n",
       name, iomask, iodata);
	...
	//printf ("TEMP DEBUG cm119_write:  %s %d %d\n", name, iomask, iodata);
 ...
 */
    ...
    if (fd < 0) {
        if (verbose) printf("[DEBUG] open() failed for %s, errno=%d\n", name, errno);
    } else {
        if (verbose) printf("[DEBUG] open() succeeded for %s, fd=%d\n", name, fd);
    }
if (fd == -1) {
    ...
    if (verbose) printf("[DEBUG] write() returned %d\n", n);
if (n != sizeof(io)) {
